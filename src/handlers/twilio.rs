...
async fn handle_from_deepgram_ws(
    ...
    let base64_encoded_mulaw =
        tts::tts(initial_assistant_message.content.clone(), state.clone()).await;
                ...
                serde_json::from_str::<deepgram_response::ServerMessageOrSo>(&msg)
            {
                if server_message_or_so.the_type == "UtteranceEnd" {
                    ...
                    send_to_subscribers(
                        streamsid.clone(),
                        "customer".to_string(),
                        running_deepgram_response.clone(),
                        state.clone(),
                    )
                    .await;
                    ...
                    let content = llm::llm(llm_messages.clone(), state.clone()).await;
                    ...
                    let base64_encoded_mulaw = tts::tts(content.clone(), state.clone()).await;
                    ...
                    // introspect the conversation
                    let llm_messages_as_string = llm::llm_messages_to_string(llm_messages.clone());
                    let introspection_prompt = { state.introspection_prompt.lock().await.clone() };
                    let introspection_prompt =
                        format!("{}: {}", introspection_prompt, llm_messages_as_string).to_string();
                    let introspection_llm_messages = vec![llm::LlmMessage {
                        role: "user".to_string(),
                        content: introspection_prompt,
                    }];
                    let introspection_content =
                        llm::llm(introspection_llm_messages.clone(), state.clone()).await;
                    println!("introspection_content: {introspection_content}");
                    let introspection_carry_on =
                        { state.introspection_carry_on.lock().await.clone() };
                    // check if the introspection feedback suggests there is an issue with the agent/assistant
                    if let Some(introspection_carry_on) = introspection_carry_on {
                        if introspection_content
                            .to_lowercase()
                            .contains(&introspection_carry_on)
                        {
                            println!("GOOD");
                        } else {
                            println!("BAD");
    ...
    for post_call_prompt in post_call_prompts {
        ...
        let futs = subscribers.iter_mut().map(|subscriber| {
            subscriber.send(
                Message::Text(
                    serde_json::to_string(&LlmResponse {
                        stream_sid: streamsid.clone(),
                        the_type: "summary".to_string(),
                        content: content.clone(),
                    })
                    .unwrap(),
                )
